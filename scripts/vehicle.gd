...
## This is used to calculate the appropriate spring rate for the wheel.
...
var prev_wheel_count := 0
var tricking := false
var trick_rotated_up := 0.0
var trick_rotated_right := 0.0
...
func initialize():
	...
	if front_brake_bias < 0.0:
		...
		var total_spring_force := front_axle_spring_force + calculate_axle_spring_force(0.4, rear_spring_length, rear_spring_rate)
		front_brake_bias = front_axle_spring_force / total_spring_force
...
func _physics_process(delta: float) -> void:
	...
	process_stability()
	process_tricks(delta)

func process_tricks(delta: float):
	var wheel_count := get_wheel_contact_count()
	var speed_met := linear_velocity.length() > speed_req
	var contacts := get_contact_count()
	
	if wheel_count == 0 and contacts == 0:
		if prev_wheel_count != 0 and speed_met:
			tricking = true
	elif wheel_count != 0 or (!speed_met and contacts != 0):
		tricking = false

	if not tricking:
		trick_rotated_right = 0.0
		trick_rotated_up = 0.0
		
	if tricking:
		var right := -throttle_input * rotation_speed * delta
		var up := steering_input * rotation_speed * delta

		trick_rotated_right += right
		trick_rotated_up += up
		
		rotate_object_local(Vector3.RIGHT, right)
		rotate_object_local(Vector3.UP, up)
	prev_wheel_count = wheel_count
